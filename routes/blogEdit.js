...
exports.blog_updateBlog=function (req,res) {
    ...
    if(req.body.role==1){   //销毁session
        ...
        var con_text_1 = req.body.con_text.toString()
        console.log(text)
        var entity = new content.Content({"name": req.session.username,"title":req.body.title,"titlePic":titlePic,"text":text,"content":con_text_1,"time":req.body.data,"cat":req.body.cat,click:0});
        entity.save();
        res.json({msg:1});
        ...
        cat.Cat.updateMany(query1,{$set:{catname:req.body.catNameNew}},function ( err,docs) {
           if(err){
               res.render('error');
           }
        });
        var query2={"name": req.session.username,"cat":req.body.catNameOld};
        content.Content.updateMany(query2,{$set:{cat:req.body.catNameNew}},function ( err,docs) {
            if(err){
                res.render('error');
            }
        });
        ...
        var text = req.body.text.toString()
        var con_text_1 = req.body.con_text.toString()
        content.Content.updateMany({"name":req.session.username,"title":req.body.title,"cat":req.body.cat},{$set:{"title":req.body.title,"content":con_text_1,"text":text,"titlePic":titlePic}},function (err, docs) {
